...
// Discord api is not implemented yet, for now. 'Force CT team' was intentionally forced off (cvar is still created), as well as server cvars management. The plugin now simply uses maps configs instead.
// Bots can have weapons when they respawn if you slay them sometimes; this is also intentional. Some maps like surf_mesa REALLY mess up with player weapons, and were causing issues.
...
Handle AnnounceTimer[MAXPLAYERS + 1] =  { null, ... };			// Tier announce timer
...
/*----------  Forwards  ----------*/
Handle g_MapFinishForward = null;
Handle g_BonusFinishForward = null;
Handle g_PracticeFinishForward = null;
...
Handle g_hRecording[MAXPLAYERS + 1] =  { null, ... };			// Client is beign recorded
Handle g_hLoadedRecordsAdditionalTeleport = null;
Handle g_hRecordingAdditionalTeleport[MAXPLAYERS + 1] =  { null, ... };
...
#include "ckSurf/misc_name_final_2hTEST32_85.sp"
#include "ckSurf/admin.sp"
#include "ckSurf/commands_rev37.sp"
#include "ckSurf/hooks_final3_colors_buy_rev743_853.sp"
#include "ckSurf/buttonpress.sp"
#include "ckSurf/sql_rev_mb4.sp"
#include "ckSurf/timer_final_revAT02_853.sp"
#include "ckSurf/replay_final5_fixed_reveCOR2_85B3.sp"
...
public void OnClientDisconnect(int client)
{
	...
	//in StopRecording & StopPlayerMimic except g_hBotMimicRecords, closed below
	//if (g_hRecordingAdditionalTeleport[client] != null)
	//{
		//CloseHandle(g_hRecordingAdditionalTeleport[client]);
		//g_hRecordingAdditionalTeleport[client] = null;
	//}
	if (IsFakeClient(client) && g_hBotMimicsRecord[client] != null)
	{
			...
			for (int i = 1; i <= MaxClients; i++)
				if (IsValidClient(i))
				{
					if (i == g_RecordBot || i == g_BonusBot)
					{
						...
						// Arm Model
						GetConVarString(g_hReplayBotArmModel, szBuffer2, 256);
						SetEntPropString(i, Prop_Send, "m_szArmsModel", szBuffer2);
						//SetEntityModel(i, szBuffer2); // That was a huge skin bug
					}
					else if (!IsFakeClient(i))
					{
						GetConVarString(g_hArmModel, szBuffer2, 256);
						SetEntPropString(i, Prop_Send, "m_szArmsModel", szBuffer2);
