...
{
	if (!IsValidClient(client) || IsFakeClient(client) || !WeAreOk)
		return;
...
{
	if (!IsValidClient(client) || IsFakeClient(client) || g_hRecording[client] == null)
		return;
	else
	{
...
public void LoadRecordReplay()
{
	...
	for (int i = 1; i <= MaxClients; i++)
	{
		...
		if (!IsPlayerAlive(i))
		{
			...
			CreateTimer(1.0, RespawnBot, GetClientOfUserId(i), TIMER_FLAG_NO_MAPCHANGE);
		}
