...
public Action RespawnBot(Handle timer, any userid)
{
	...
	{
		if (g_hBotMimicsRecord[client] != null && !IsPlayerAlive(client) && IsFakeClient(client) && (GetClientTeam(client) >= CS_TEAM_T))
		{
	...
	if (g_RecordBot != -1)
	{
		if (IsValidClient(g_RecordBot))
		{
			...
			//{
				//char szBuffer3[256];
				//char szBuffer4[256];
				//GetConVarString(g_hReplayBotPlayerModel, szBuffer3, 256);
				//SetEntityModel(g_RecordBot, szBuffer3);
				//CreateTimer(0.3, SetThoseSkinsNow, GetClientUserId(g_RecordBot), TIMER_FLAG_NO_MAPCHANGE);

				//GetConVarString(g_hReplayBotArmModel, szBuffer4, 256);
				//SetEntPropString(g_RecordBot, Prop_Send, "m_szArmsModel", szBuffer4);
				//CreateTimer(0.5, SetArmsModel, GetClientUserId(g_RecordBot), TIMER_FLAG_NO_MAPCHANGE);
			//}
	...
	if (g_BonusBot != -1)
	{
		if (IsValidClient(g_BonusBot))
		{
			...
			//{
				//char szBuffer5[256];
				//char szBuffer6[256];
				//GetConVarString(g_hReplayBotPlayerModel, szBuffer5, 256);
				//SetEntityModel(g_BonusBot, szBuffer5);
				//CreateTimer(0.3, SetThoseSkinsNow, GetClientUserId(g_BonusBot), TIMER_FLAG_NO_MAPCHANGE);
				//
				//GetConVarString(g_hReplayBotArmModel, szBuffer6, 256);
				//SetEntPropString(g_BonusBot, Prop_Send, "m_szArmsModel", szBuffer6);
				//CreateTimer(0.5, SetArmsModel, GetClientUserId(g_BonusBot), TIMER_FLAG_NO_MAPCHANGE);
			//}
	...
	if (g_hBotMimicsRecord[client] != null)
	{
		...
		if (g_BotMimicTick[client] == 0)
		{
			...
			if (client == g_RecordBot && g_bNewRecordBot)
			{
				...
				//PrintToServer("[CK] ---------------- g_bNewRecordBot = false ----------------");
				//if(GetConVarBool(g_hPlayerSkinChange))
				//{
					//This will avoid weird custom models animations, they need to be slayed, not only respawned
					//ForcePlayerSuicide(client);
					//CreateTimer(5.0, RespawnBot, GetClientUserId(client), TIMER_FLAG_NO_MAPCHANGE); // Only if autorespawn is off for some reason
				//}
			}
			
			if (client == g_BonusBot && g_bNewBonusBot)
			{
				g_bNewBonusBot = false;
				//PrintToServer("[CK] ---------------- g_bNewBONUSBot = false ----------------");
				//if(GetConVarBool(g_hPlayerSkinChange))
				//{
					//This will avoid weird custom models animations
					//ForcePlayerSuicide(client);
					//CreateTimer(5.5, RespawnBot, GetClientUserId(client), TIMER_FLAG_NO_MAPCHANGE); // Only if autorespawn is off for some reason
				//}
			}
		}
		else
