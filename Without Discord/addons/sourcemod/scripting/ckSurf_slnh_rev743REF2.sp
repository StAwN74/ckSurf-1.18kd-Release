				...
				if (IsValidClient(i))
				{
					if (IsFakeClient(i))
					{
						if (i == g_RecordBot)
						{
							...
							ServerCommand("bot_quota %i", GetConVarInt(hBotQuota1)-1, false, false);
...
public void OnPluginStart()
{
	...
	gcv_sFlagString = CreateConVar("hl_goto_flags", "a", "Flag for sm_goto and sm_bring command access (default is vip)", FCVAR_NOTIFY, true, 0.0, true, 1.0);
...
public Action Event_PlayerDisconnect(Handle event, const char[] name, bool dontBroadcast)
{
	...
	if (IsValidClient(client))
	{
		if (IsFakeClient(client))
		{
			//This msg doesn't display anymore -> Happy (bots get released from their task properly now)
