		...
		if (!IsFakeClient(client))
		{
			...
			if (g_hBotMimicsRecord[client] != null)
			{
				...
				if (GetClientTeam(client) >= CS_TEAM_T)
					CreateTimer(1.0, RespawnBot, GetClientOfUserId(client), TIMER_FLAG_NO_MAPCHANGE);
			}
	}
	//return Plugin_Continue;
	return;
...
//{
	//Blocking late load during warmup so I remove this
	//if (WeAreOk && reason == CSRoundEnd_GameStart)
		//return Plugin_Handled;
	//int timeleft;
	//GetMapTimeLeft(timeleft);
	//Beware that plugin now uses maps configs with mp_ignore_round_win_conditions, so I removed this too
