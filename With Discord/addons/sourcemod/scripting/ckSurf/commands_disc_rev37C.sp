public Action Command_Bug(int client, int args)
{
	if (client < 1)
	{
		PrintToServer("You must be in game to use the command");
...
public void ReportBugMenu(int client)
{
	...
	AddMenuItem(menu, "ckSurf Bug", "ckSurf Bug");
...
public int ReportBugHandler(Menu menu, MenuAction action, int param1, int param2)
{
	if (action == MenuAction_Select)
	{
		...
		CPrintToChat(param1, "Type your message");
	}
	...
	switch (action)
	{
		case MenuAction_Select:
		{
			switch (item)
			{
				...
				{
					toggleTrail(client);
					CreateTimer(0.1, RefreshVIPMenu, GetClientUserId(client), TIMER_FLAG_NO_MAPCHANGE);
				}
				case 1:
				{
					CreateTimer(0.1, RefreshVIPMenu, GetClientUserId(client), TIMER_FLAG_NO_MAPCHANGE);
...
public int H_PlayersTitles(Menu tMenu, MenuAction action, int client, int item)
{
	switch (action)
	{
		case MenuAction_Select:
		{
			...
			CreateTimer(0.5, SetClanTag, GetClientUserId(client), TIMER_FLAG_NO_MAPCHANGE);
...
public Action Command_Teleport(int client, int args)
{
	...
	if (client < 1)
	{
		//PrintToServer("Client must be in game to use Teleport command");
		return Plugin_Handled;
	}
...
public Action Command_Restart(int client, int args)
{
	if (client < 1)
	{
		return Plugin_Handled; // PrintToServer("Client must be in game to use the Restart command");
	}
...
{
	if (IsValidClient(client)) //the second condition is only checked if the first passes - wrong with IsClientIngame put right before IsfakeClient, for example. Log error (client not ingame)
		if (g_bTierFound[0])
...
public int ChallengeMenuHandler2(Menu menu, MenuAction action, int param1, int param2)
{
	if (action == MenuAction_Select)
	{
		...
		for (int i = 1; i <= MaxClients; i++)
		{
			if (IsValidClient(i))
			{
				 if (IsPlayerAlive(i) && i != param1 && !IsFakeClient(i))
				{
...
public Action Client_Accept(int client, int args)
{
	...
	for (int i = 1; i <= MaxClients; i++)
	{
		if (IsValidClient(i))
		{
			if (IsPlayerAlive(i) && i != client && g_bChallenge_Request[i])
			{
				if (StrEqual(szSteamId, g_szChallenge_OpponentID[i]))
				{
					...
					CreateTimer(1.0, Timer_Countdown, GetClientUserId(i), TIMER_REPEAT | TIMER_FLAG_NO_MAPCHANGE);
					CreateTimer(1.0, Timer_Countdown, GetClientUserId(client), TIMER_REPEAT | TIMER_FLAG_NO_MAPCHANGE);
					...
					CreateTimer(1.0, CheckChallenge, GetClientUserId(i), TIMER_REPEAT | TIMER_FLAG_NO_MAPCHANGE);
					CreateTimer(1.0, CheckChallenge, GetClientUserId(client), TIMER_REPEAT | TIMER_FLAG_NO_MAPCHANGE);
				}
			}
		}
	}
			...
			if (IsValidClient(i))
			{
				if (i != client)
				{
					...
					if (StrEqual(szSteamIdOpponent, g_szChallenge_OpponentID[client]))
					{
						...
						//db update
						CreateTimer(0.1, UpdatePlayerProfile, GetClientUserId(i), TIMER_FLAG_NO_MAPCHANGE);
...
public void SpecPlayer(int client, int args)
{
	...
	if (args == 0)
	{
		...
		if (g_RecordBot != -1)
		{
			...
			{
				if (g_RecordBot != -1 && IsPlayerAlive(g_RecordBot))
				{
		...
		if (g_BonusBot != -1)
		{
			...
			{
				if (g_BonusBot != -1 && IsPlayerAlive(g_BonusBot))
				{
		...
		for (int i = 1; i <= MaxClients; i++)
		{
			if (IsValidClient(i))
			{
				if (IsPlayerAlive(i) && i != client && !IsFakeClient(i))
				{
					if (count == 0)
					{
						...
						for (int x = 1; x <= MaxClients; x++)
						{
							if (IsValidClient(x))
							{
								if (IsPlayerAlive(x) && x != client && !IsFakeClient(x) && g_PlayerRank[x] > 0)
								{
...
public void PauseMethod(int client)
{
	...
	{
		if (GetConVarBool(g_hPauseServerside) == false && client != g_RecordBot && client != g_BonusBot && client != g_InfoBot)
		{
...
public void GotoMethod(int client, int target)
{
	...
	CPrintToChat(client, "Please use sm_goto or !goto instead of sm_gototest or !gototest");
	
	if (IsValidClient(target))
		CPrintToChat(target, "Please ask the mate to use sm_goto or !goto instead of !gototest");
}
...
public void Action_NoClip(int client)
{
	if (IsValidClient(client))
	{
		if (!IsFakeClient(client) && IsPlayerAlive(client) && GetConVarBool(g_hNoClipS))
		{
...
public int HelpPanel2(int client)
{
	...
	DrawPanelText(panel, "!gototest [<name>] - teleports you to a given player");
