...
public void Event_OnPlayerSpawn(Handle event, const char[] name, bool dontBroadcast)
{
	...
	else
	{
		...
		if (GetConVarBool(g_hPlayerSkinChange) && (GetClientTeam(client) > 1))
		{
			char szBuffer7[256];
			//char szBuffer8[256];
			GetConVarString(g_hArmModel, szBuffer7, 256);
			SetEntPropString(client, Prop_Send, "m_szArmsModel", szBuffer7);

			//GetConVarString(g_hPlayerModel, szBuffer8, 256);
			//SetEntityModel(client, szBuffer8);
			CreateTimer(0.3, SetArmsModel, GetClientUserId(client), TIMER_FLAG_NO_MAPCHANGE);
		}
