	...
	else
	{
		...
		if (IsFakeClient(client))
		{
			if (GetConVarBool(g_hPlayerSkinChange) && GetClientTeam(client) > 1)
			{
				...
				char szBuffer12[256];
				// Bot Player Model
				GetConVarString(g_hReplayBotPlayerModel, szBuffer11, 256);
				SetEntityModel(client, szBuffer11);
				//CreateTimer(0.3, SetThoseSkinsNow, GetClientUserId(client), TIMER_FLAG_NO_MAPCHANGE);
				// Bot Arm Model
				GetConVarString(g_hReplayBotArmModel, szBuffer12, 256);
				SetEntPropString(client, Prop_Send, "m_szArmsModel", szBuffer12);
				//CreateTimer(0.5, SetArmsModel, GetClientUserId(client), TIMER_FLAG_NO_MAPCHANGE);
			}
			...
			return;
			//return Plugin_Continue;
			//Are you sure, if you return here, they will not get skins, etc. - done above
		}
		...
		if (GetConVarBool(g_hPlayerSkinChange) && (GetClientTeam(client) > 1))
		{
			char szBuffer7[256];
			char szBuffer8[256];
			GetConVarString(g_hPlayerModel, szBuffer7, 256);
			SetEntityModel(client, szBuffer7);
			//CreateTimer(0.3, SetThoseSkinsNow, GetClientUserId(client), TIMER_FLAG_NO_MAPCHANGE);
			
			GetConVarString(g_hArmModel, szBuffer8, 256);
			SetEntPropString(client, Prop_Send, "m_szArmsModel", szBuffer8);
			//CreateTimer(0.5, SetArmsModel, GetClientUserId(client), TIMER_FLAG_NO_MAPCHANGE);
		}
...
public Action Say_Hook(int client, const char[] command, int argc)
{
	...
	if (IsValidClient(client))
	{
		...
		//BugMsg - Double send?
		//if (StrContains(sText, "!bug", false) == 0)
		//{
			//Format(g_sBugMsg[client], sizeof(g_sBugMsg), sText);
			//SendBugReport(client);
		//}
		...
		if (IsFakeClient(client))
		{
			if (g_hBotMimicsRecord[client] != null)
			{
				...
				if (GetClientTeam(client) >= CS_TEAM_T && client != g_InfoBot)
					CreateTimer(5.0, RespawnBot, GetClientUserId(client), TIMER_FLAG_NO_MAPCHANGE); // Let ragdoll dissolve and autorespawn first
			}
		}
	}
	//return Plugin_Continue;
...
public Action DelayedStuff2(Handle timer)
{
	...
	{
		CreateTimer (4.0, RefreshBonusBot2, _, TIMER_FLAG_NO_MAPCHANGE);
	}
