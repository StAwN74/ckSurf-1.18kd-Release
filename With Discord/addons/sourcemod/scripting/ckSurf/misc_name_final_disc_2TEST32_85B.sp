void disableServerHibernate()
{
	...
	{
		PrintToServer("[ckSurf] Disabling server hibernation for database access.");
	...
	if (IsValidClient(client))
	{
		...
		if (g_bMapSRVRecord[client])
		{
			...
			if (!StrEqual(buffer, ""))
			{
				...
				CPrintToChat(client, "{gold}Your record has been announced on Discord.");
			}
		}
...
public void sendDiscordAnnouncement(char szName[32], char szMapName[128], char szTime[32])
{
	...
	hook.SetUsername("ckSurf Records");
	...
	Format(szMessage, sizeof(szMessage), "%s has beaten the %s map record on %s server with a time of %s", szName, szMapName, g_szServerName, szTime);
...
public void SendBugReport(int client)
{
	...
	hook.SetUsername("ckSurf Bugtracker");

	MessageEmbed Embed = new MessageEmbed();

	// Format Title
	char sTitle[256];
	Format(sTitle, sizeof(sTitle), "Bug Type: %s || Server: %s || Map: %s", g_sBugType[client], g_szServerName, g_szMapName);
	...
	CPrintToChat(client, "Bug report sent");
}
...
public void CallAdmin(int client, char[] sText)
{
	...
	hook.SetUsername("ckSurf Calladmin");

	MessageEmbed Embed = new MessageEmbed();

	// Format title
	char sTitle[256];
	Format(sTitle, sizeof(sTitle), "Server: %s || Map: %s", g_szServerName, g_szMapName);
	Embed.SetTitle(sTitle);
	...
	CPrintToChat(client, "Report sent");
}
