...
public void Event_OnPlayerSpawn(Handle event, const char[] name, bool dontBroadcast)
{
	...
	if (IsValidClient(client))
	{
		...
		//strip weapons
		if ((GetClientTeam(client) > 1) && IsValidClient(client) && !IsFakeClient(client))
		{
...
{	
	//Idk why we need to keep this empty line, but I had a bug
}
...
public void Event_OnPlayerTeam(Handle event, const char[] name, bool dontBroadcast)
{
	...
	//Trying to check if bot here, and kick him to fix surf_summer double bot, but it may spam console
	//if (IsFakeClient(client) && (GameStartBool))
	//{
		//KickClient(client);
		//return Plugin_Continue;
		//return;
...
public Action Event_OnRoundEnd(Handle event, const char[] name, bool dontBroadcast)
{
	...
	//LoadReplays();
	//LoadInfoBot();
	//CreateTimer(0.1, DelayedStuff2, INVALID_HANDLE, TIMER_FLAG_NO_MAPCHANGE);
	
	//Setting it to true means that a GameRestart is incoming
	//GameStartBool = true;
...
public Action Event_OnMatchStartCS(Handle event, const char[] name, bool dontBroadcast)
{
...
public Action DelayedStuff2(Handle timer)
{
	...
	PrintToServer("[CK] All Bots reloaded. Surf_summer might spawn an invisible bot, but no crash.");
}
